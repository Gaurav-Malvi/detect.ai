...
def main():
    ...
    st.title("🔍 Detect.ai")
    st.write("Upload an image, video or audio to check if it's real or fake using multiple deep learning models.")
    
    # Input type selector
    input_type = st.radio("Select input type:", ["Image", "Video", "Audio"], horizontal=True)
    ...
    if input_type == "Image":
        ...
        MODEL_PATH = "C:/Users/disha/Desktop/AudioDeepFakeDetection-main/AudioDeepFakeDetection-main/models/random_forest_model.joblib"
        loaded_model = joblib.load(MODEL_PATH)

        # Function to extract features from an audio file
        def extract_features(file_path, target_sr=22050, duration=3):
            try:
                audio, sample_rate = librosa.load(file_path, sr=target_sr, mono=True, dtype=np.float32)
                ...
                mfccs = np.mean(librosa.feature.mfcc(y=audio, sr=target_sr, n_mfcc=40).T, axis=0)
                return mfccs
            except Exception as e:
...
# Function to predict deepfake audio
        def runtest(file):
            example_features = extract_features(file)
        ...
        if uploaded_file:
            ...
            temp_file_path = f"temp_audio.wav"
            with open(temp_file_path, "wb") as f:
                f.write(uploaded_file.getbuffer())

            # Run detection
            if st.button("Run Detection"):
                result = runtest(temp_file_path)
                st.success(f"🔍 Prediction: **{result} Audio File**")

            # Cleanup temp file
            os.remove(temp_file_path)

        # # Model accuracy checkbox (Dummy example)
        # if st.checkbox("Show Model Accuracy"):
        #     st.info("✅ Model Accuracy: **85.6%** (Example)")
...
def process_video_input(video_file):
    try:
        ...
        # Load models
        converted_dir = "C:/Users/disha/Downloads/deepfake-detection-project-v4-main/deepfake-detection-project-v4-main/converted_models"
